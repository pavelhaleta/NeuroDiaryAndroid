...
class TableRealContent : SQLTable("RealContent"){
    //table fields
    ...
    var recordId: Int
        get() {
            return _recordId.value as Int
        }
        set(value) {
            _recordId.value = value
        }
    ...
    companion object{
        ...
        fun toList(db: SQLiteDatabase, whereClause: String): ArrayList<TableRealContent>?{
            ...
            val list = ArrayList<TableRealContent>()
            cursor.moveToFirst()
            for (i in 0 until cursor.count){
                val element = TableRealContent()
