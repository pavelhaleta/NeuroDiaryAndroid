...
class TableAction : SQLTable("Action") {
    //table fields
    ...
    companion object{
        const val tableName = "Action"

        fun toList(db: SQLiteDatabase, whereClause: String): ArrayList<TableAction>?{
            ...
            val list = ArrayList<TableAction>()
            cursor.moveToFirst()
            for (i in 0 until cursor.count){
                val element = TableAction()
