...
import com.pavelhaleta.neurodiary.model.Action
import com.pavelhaleta.neurodiary.model.Goal

class TableGoal : SQLTable("Goal") {
    //table fields
    ...
    override fun initialData(db: SQLiteDatabase) {
        for (value in Goal.values()){
            db.insert(this.tableName, null,ContentValues().also {
                it.put("id", value.id)
                it.put(_name.name, value.value)
            }).toInt()
        }
    }
    ...
    companion object{
        ...
        fun toList(db: SQLiteDatabase, whereClause: String): ArrayList<TableGoal>?{
            ...
            val list = ArrayList<TableGoal>()
            cursor.moveToFirst()
            for (i in 0 until cursor.count){
                val element = TableGoal()
